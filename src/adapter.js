...
const { stableJson, cloneJson, byDiagnostic, byObjectId, compareText, isCanonicalObjectId, isJsonValue, requireObjectId } = require("./memory-store");
...
function requireQuery(query) {
  if (query === null || typeof query !== "object" || Array.isArray(query)) throw new InvalidRequestError("query must be an object");
...
function normalizeCandidate(method, candidate, index) {
  if (candidate === null || typeof candidate !== "object" || Array.isArray(candidate) || !hasOwn(candidate, "objectId") || !hasOwn(candidate, "object")) {
    invalidStoreResponse(method, "contains an invalid Candidate", { index });
  }
  if (!isCanonicalObjectId(candidate.objectId)) invalidStoreResponse(method, "contains a non-canonical ObjectId", { index });
...
function normalizeWarnings(method, warnings) {
  for (let index = 0; index < warnings.length; index += 1) {
    ...
    if (warning === null || typeof warning !== "object" || Array.isArray(warning) || typeof warning.code !== "string" || !isJsonValue(warning)) {
...
function normalizeEnvelope(method, envelope) {
  if (envelope === null || typeof envelope !== "object" || Array.isArray(envelope)) invalidStoreResponse(method, "is not an object");
  ...
  if (!Array.isArray(envelope.provenance) || !isJsonValue(envelope.provenance)) invalidStoreResponse(method, "has invalid provenance");
  ...
  for (let index = 0; index < envelope.objects.length; index += 1) {
    ...
    if (objects.has(candidate.objectId) && stableJson(prior) !== stableJson(candidate.object)) {
      throw new StoreIntegrityError("store discovery returned conflicting representations for one ObjectId", { objectId: candidate.objectId });
    }
  ...
  return {
    ...
    objects: [...objects.entries()].map(([objectId, object]) => ({ objectId, object: cloneJson(object) })).sort(byObjectId),
...
function storeWarnings(envelopes) {
  const incomplete = envelopes.filter(({ envelope }) => !envelope.complete).map(({ method }) => method).sort(compareText);
  const forwarded = envelopes.flatMap(({ envelope }) => envelope.warnings.map((warning) => cloneJson(warning)));
  if (incomplete.length) forwarded.push({ code: "W_STORE_DISCOVERY_INCOMPLETE", message: "one or more Store discovery operations are incomplete", detail: { methods: incomplete } });
...
async function resolveAliasFromStore(query, store, options = {}) {
  if (options === null || typeof options !== "object" || Array.isArray(options)) throw new InvalidRequestError("options must be an object");
  if (options.discovery !== undefined && options.discovery !== null && (typeof options.discovery !== "object" || Array.isArray(options.discovery))) {
